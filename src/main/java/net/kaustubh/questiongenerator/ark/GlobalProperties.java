...
public class GlobalProperties {
	...
	public static Properties getProperties(){
		if(properties == null){
			String defaultPath = GlobalProperties.class.getClassLoader().getResource("config/QuestionTransducer.properties").getPath();
		...
		try{
			...
			for (Object key: properties.keySet()) {
				...
				if (Val.startsWith("config"))
					properties.setProperty(key.toString(), GlobalProperties.class.getClassLoader().getResource(Val).getPath());
			}
		}catch(Exception e){
			e.printStackTrace();
			System.exit(0);
